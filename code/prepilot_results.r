...
data = fread("F:/001_Learn_UCB/241_Experiments_and_Causality/final_project/20171025_results.csv")
...
end = 13 # check excel sheet for last row of valid responses
subject.row_range = c(3:end) # valid responses from row 3 to end
n = end - 3 + 1

# unchecked responses in breed names
dt.raw_responses = data[subject.row_range, 22:71]  #from TQ1 to TQ48

# check responses and store in new frame
dt.correct_responses = data.table(ID = data[subject.row_range,]$ResponseId)
for (i in 1:50) {
  col.name = names(dt.raw_responses)[i]
  answer = ans[i]
  newcol = as.numeric(dt[, get(col.name)] == answer)
