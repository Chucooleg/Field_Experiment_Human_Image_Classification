...
A note on supplementary files: A clean, well organized Github repo is available for this project. Access to raw, intermediate and transformed data is available for the evaluation of this project in the context of course w241 upon request. In addition, many of the programming code displayed in the following sections have been abstracted -- extensive R functions have been written to transform the raw data for statistical modeling and management of our AMT turkers. More R functions have been written to aid inferences. In general, code blocks that are non-analysis related are hidden and only visible in the submitted R file.
...
pilot_summary = data.frame(Name=c("Pilot", "Pilot"), Posting=c(1,2), Treatmet=c("$0.10","$0.25"),
                           N=c(nrow(worker_perf_pilot_0.10),nrow(worker_perf_pilot_0.25)),
                           TotalTimeTaken=c("2h 30min","1h 20min"), 
                           AvgTimePerTaskMin = c(avg_time_010,avg_time_025), 
                           AccuracyMean=c(smry_010$mean,smry_025$mean), AccuracySE=c(smry_010$se,smry_025$se))
knitr::kable(pilot_summary, "markdown")
```

(think about the table a bit more)
